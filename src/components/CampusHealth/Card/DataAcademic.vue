<template>
  <div class="rounded-3xl bg-base-white border border-[#ECECEC] p-6">
    <!-- Header -->
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-4">
        <div
          class="w-16 h-16 rounded-full bg-[#F5F5F5] flex items-center justify-center"
        >
          <component
            :is="icon"
    ...
    <div class="space-y-6">
      <div
        ...
        :key="index"
        :class="row.fullWidth ? '' : 'grid grid-cols-2 gap-5'"
      >
        <!-- Full Width -->
        <template v-if="row.fullWidth">
          <div>
            ...
            <InputText
              ...
              :name="`academic-${index}`"
              inputClass="!h-20 !rounded-3xl !border-[#D8D8D8] !px-6 !text-[28px] !text-[#A8A8A8] !bg-transparent"
            />

            <p
              v-if="row.suffix"
              class="mt-2 text-lg font-medium text-primary-900"
            >
              {{ row.suffix }}
            </p>
          </div>
        ...
        <template v-else>
          <div
            v-for="(field, idx) in row.fields"
            :key="idx"
          >
            <p class="text-[18px] font-medium text-primary-900 mb-3">
              {{ field.label }}
            </p>
