<template>
  <div class="relative grid gap-1">
    ...
    <div class="relative">
      <i class="absolute left-3 top-2" :class="`${icon} ${modelValue ? 'text-primary-500' : 'text-text-300'}`" v-if="!$slots.icon && icon"></i>
      <span v-else-if="$slots.icon" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 [&>svg]:w-full [&>svg]:h-full" :class="modelValue ? 'text-primary-500' : 'text-text-300'">
        <slot name="icon" />
      </span>
      ...
      <div class="cursor-pointer absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 [&>svg]:w-full [&>svg]:h-full text-text-300" @click="showPassword = !showPassword">
        <slot name="toggle-icon" :visible="!showPassword">
